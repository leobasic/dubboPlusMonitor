        ...
        registry.subscribe(subscribeUrl, new NotifyListener() {
            public void notify(List<URL> urls) {
                ...
                for (URL url : urls) {
                    ...
                    if (Constants.PROVIDERS_CATEGORY.equals(category)) {
                        ...
                        if (Constants.EMPTY_PROTOCOL.equals(url.getProtocol())) {
                           ...
                           // if ( proivderMap.get(service)==null||proivderMap.get(service).size()==0 ) services.remove(service);
                        } else {
                ...
                if (proivderMap != null && proivderMap.size() > 0) {
                    ...
                    //System.err.println("-------> serviceProviders:  "+serviceProviders);
                }
                if (consumerMap != null && consumerMap.size() > 0) {
                    serviceConsumers.putAll(consumerMap);
                    
                    //System.err.println("++++++++> serviceConsumers:  "+serviceConsumers);
                }
                //过滤掉没有privoder和consumer
                for (String service:services)
                {
                	if (serviceProviders.get(service)==null && serviceConsumers.get(service)==null)
                	{
